...
module Make (Job : MapReduce.Job) =
  struct
    ...
    (* total number of all workers that have ever been added. Remain unchanged when remove_worker is called *)
    let add_worker (r, w) =
      let id = !size
      in
        (size := !size + 1;
    ...
    let connect_to_all (lst : (string * int) list) : unit Deferred.t =
      ...
      let connect host port =
        (Tcp.connect (Tcp.to_host_and_port host port)) >>=
          (fun (_, r, w) ->
             (Writer.write_line w name; return (add_worker (r, w)))) in
      let rec helper =
        function
        | [] -> return ()
   ...
   * return the reduce result *)
      ...
      (WRequest.send w (WRequest.ReduceRequest (k, interLst));
       (WResponse.receive r) >>=
         (function
          ...
          | _ ->
              (remove_worker (r, w);
               reduce_job (k, interLst) (get_next_worker ()))))
    ...
    let map_reduce inputs =
      (connect_to_all !hosts_ports) >>=
        (fun () ->
           (((map inputs) >>| List.flatten) >>| C.combine) >>=
